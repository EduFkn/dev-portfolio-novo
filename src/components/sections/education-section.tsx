  ...
  return (
    <section id="education" className="bg-card/10">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="scroll-snap-container hide-scrollbar space-y-4 md:space-y-6"> {/* Applied hide-scrollbar */}
          {educationData.map((edu, index) => (
            ...
            >
              <div className="md:w-2/5 p-3 md:p-4 border-b md:border-b-0 md:border-r border-border"> {/* Reduced padding */}
                <CardHeader className="p-0 mb-2"> {/* Reduced margin */}
                  <div className="flex items-center text-primary mb-1">
                    <GraduationCap className="h-4 w-4 md:h-5 md:w-5 mr-1.5" />
                    <CardTitle className="font-headline text-sm md:text-base text-accent leading-tight">{edu.degree}</CardTitle> {/* Reduced font size */}
                  </div>
                ...
                <CardContent className="p-0">
                  <CardDescription className="mb-1.5 text-foreground/80 text-[0.7rem] md:text-xs leading-snug line-clamp-2 md:line-clamp-3">{edu.description}</CardDescription> {/* line-clamp and leading-snug */}
                  {edu.highlights && edu.highlights.length > 0 && (
                    <ul className="list-disc list-inside space-y-0.5 text-foreground/70 text-[0.7rem] md:text-xs"> {/* Reduced space-y and font size */}
              ...
              <div className="md:w-3/5 p-3 md:p-4"> {/* Reduced padding */}
                <h4 className="font-headline text-[0.75rem] md:text-sm text-foreground/90 mb-2 flex items-center"> {/* Reduced font size and margin */}
